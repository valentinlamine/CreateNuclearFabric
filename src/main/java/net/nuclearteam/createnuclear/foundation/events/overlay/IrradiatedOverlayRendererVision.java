...
public class IrradiatedOverlayRendererVision {
    ...
    public static void renderOverlay(GuiGraphics graphics, float partialTicks) {
        ...
        GameType playerMode = mc.interactionManager.getCurrentGameMode();
        ...
        if (mc.options.hudHidden || playerMode == GameType.SPECTATOR || playerMode == GameType.CREATIVE) {
            irradiatedVisionAlpha = 0.0f;
            return;
        }
        LocalPlayer localPlayer = mc.player;
        RenderSystem.enableBlend();
        if (localPlayer.hasStatusEffect(CNEffects.RADIATION.get())) {
    ...
    public static void renderTextureOverlay(GuiGraphics guiGraphics, ResourceLocation texture, float alpha, boolean onlyFirstPerson) {
        ...
        guiGraphics.setShaderColor(1f, 1f, 1f, alpha);
        guiGraphics.drawTexture(texture, 0, 0, -90, 0.0f, 0.0f, guiGraphics.getScaledWindowWidth(), guiGraphics.getScaledWindowHeight(), guiGraphics.getScaledWindowWidth(), guiGraphics.getScaledWindowHeight());
